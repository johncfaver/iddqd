...
//Perform search in database. Place resulting molids into session.
...
//Query parameters-constants
    ...
    //Max Tanimoto coefficient for structure similarity search.
 	$similaritythreshold=(isset($_GET['similaritythreshold']))?(real)$_GET['similaritythreshold']:0.4;
      ...
      //Save query structure to /tmp
        ...
        if($searchtype=='similarity'){
            ...
            while(count($oarray)<$njobs){
                $ijob = count($oarray);
                for($i=$ijob*$nworkers;$i<($ijob+1)*$nworkers;$i++){
                    if($i>$njobs){
                         break;
                    }
                    $cmdstr = 'for i in';
