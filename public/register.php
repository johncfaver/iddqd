...
//Handles user registration.
    ...
    if(!isset($_POST['desiredusername'])){
        header('Location: registerpage.php?invitekey='.$invitekey.'&usernameisbad=1');
        exit;
    }
	$desiredusername = pg_escape_string($_POST['desiredusername']);

    if(!isset($_POST['desiredpassword'])){
        header('Location: registerpage.php?invitekey='.$invitekey.'&passwordisbad=1');
        exit;
    }
	$desiredpassword = pg_escape_string($_POST['desiredpassword']);

    if(!isset($_POST['desiredemail'])){
        header('Location: registerpage.php?invitekey='.$invitekey.'&emailisbad=1');
        exit;
    }
...
//Check for previous registration       
	...
	if($q->rowCount()!=0){
        $dbconn=null;
		header("Location: registerpage.php?invitekey='.$invitekey.'&nameexists=1");		
        exit;
	}
