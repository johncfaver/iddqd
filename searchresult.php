<?php
	...
	if($query_structure){
		...
		if($searchtype=='similarity'){
			$similarities = Array();
			$cmdstr='for i in $(seq '.$dbminmol.' 1 '.$dbmaxmol.');do /usr/local/bin/babel /home/faver/ddb/dev/uploads/structures/$i.mol '.$tempmolfile.' -ofpt 2>/dev/null | ';
			...
			for($i=$dbminmol;$i<=$dbmaxmol;$i++){
				$cmdstr='/usr/local/bin/babel /home/faver/ddb/dev/uploads/structures/'.$i.'.mol -s'.$tempmolfile.' /tmp/'.session_id().'results.smi -xt 2>/dev/null;';
